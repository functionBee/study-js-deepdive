...
- **값(Value):**
    ...
    - 값은 변수에 할당되거나 함수에 인수로 전달되거나 표현식과 연산에 사용됩니다.
    - 값은 숫자, 문자열, 부울, 배열, 객체 등과 같은 다양한 유형을 가질 수 있습니다.
    - 값은 동적으로 변경될 수 있으며 프로그램 실행 중에 변할 수 있습니다.

- **리터럴(Literal):**

    - 리터럴은 소스 코드에서 특정 유형의 특정 값을 직접 표현하기 위해 사용되는 표기법 또는 구문입니다.
    - 리터럴은 값을 더 직접적이고 편리하게 표현하기 위해 사용됩니다.
    - 리터럴은 값의 고정된 정적인 표현이며, 프로그램 실행 중에 변경되지 않습니다.
    - 리터럴은 해당하는 값의 유형에 따라 사용됩니다.
    - 리터럴 유형:
    
    |   	| 리터럴의 유형    	| 의미                                           	| 예                                	|
    |---	|------------------	| -----------------------------------------	| ---------------------------	|
    | 1 	| 숫자 리터럴      	| 숫자 값을 나타냅니다.                           	| let age = 25;                     	|
    | 2 	| 문자열 리터럴    	| 문자(character)들의 연속을 나타냅니다.          	| let name = "John Doe";            	|
    | 3 	| 부울 리터럴      	| 부울 값(true 또는 false)을 나타냅니다.          	| let isStudent = true;             	|
    | 4 	| 배열 리터럴      	| 대괄호로 둘러싸인 값들의 목록을 나타냅니다.     	| let numbers = [1, 2, 3, 4, 5];    	|
    | 5 	| 객체 리터럴      	| 중괄호로 둘러싸인 키-값 쌍의 모음을 나타냅니다. 	|                                   	|
    | 6 	| 정규식 리터럴    	| 문자열 패턴을 나타내기 위해 사용됩니다.         	| let pattern = /abc/;              	|
...
- **표현식(Expression):**
    - 표현식은 값으로 평가되는 코드 단위입니다.
    - 표현식은 변수, 상수, 리터럴, 연산자, 함수 호출 등의 조합으로 이루어집니다.
    - 표현식은 값을 생성하거나 값을 가지고 연산을 수행합니다.
    - 예시: `2 + 3`, `x > 5`, `"Hello, " + name`
