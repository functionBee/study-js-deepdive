...
- **변수(Variable)**
    ...
    > **동적 언어(Dynamic language)란?** <br> 
    > 프로그램이 실행되는 동안 타입 검사(type checking)와 같은 작업을 수행하는 것이 아니라,<br>
    > 런타임 (runtime) 시에 타입과 관련된 결정을 내리는 프로그래밍 언어를 의미합니다.<br>
    > 동적 언어는 변수의 타입이 프로그램 실행 중에 동적으로 바뀔 수 있으며, 변수에는 다양한 타입의 값을 할당할 수 있습니다.<br>
