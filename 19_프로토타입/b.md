...
- **명령형 프로그래밍**: 명령형 프로그래밍은 프로그램이 특정 상태에 도달하기 위해 수행해야 할 단계나 동작을 설명하는 것에 중점을 둡니다. JavaScript에서는 동작을 수행하거나 프로그램 상태를 직접 수정하는 명령문을 사용하여 명령형 코드를 작성할 수 있습니다.
  ```jsx
  let x = 5;
  if (x > 0) {
    console.log("양수");
...
- **함수형 프로그래밍**: 함수형 프로그래밍은 계산을 수학적 함수의 평가로 다루는 프로그래밍 패러다임입니다. JavaScript에서는 함수를 일급 객체로 사용하여 함수형 코드를 작성할 수 있습니다. 이는 함수를 인자로 전달하거나 함수를 반환하거나 변수에 할당할 수 있다는 것을 의미합니다.
  ```jsx
...
- **프로토타입 기반 프로그래밍**: 프로토타입 기반 프로그래밍은 기존 객체를 복제하여 새로운 객체를 생성하는 객체지향 프로그래밍 스타일입니다. JavaScript에서는 생성자 함수나 객체 리터럴을 사용하여 객체를 생성하고, Object.create() 메서드를 사용하여 이러한 객체를 프로토타입으로 사용하여 새로운 객체를 생성할 수 있습니다.
  ...
  person.name = "John";
  person.greet(); // 출력: 안녕하세요, John님!
  ```

- **객체지향 프로그래밍 (OOP)**: 객체지향 프로그래밍은 데이터와 그 데이터를 조작하는 코드를 포함하는 객체 개념을 기반으로 하는 프로그래밍 패러다임입니다. JavaScript에서는 생성자 함수, 클래스 (ECMAScript 2015에서 도입됨), 객체 리터럴을 사용하여 객체를 생성할 수 있습니다.
  ...
  console.log(rectangle.getArea()); // 출력: 15
  ```

  > 참고: ES6 (ECMAScript 2015)의 도입으로 JavaScript는 클래스 문법을 사용하여 객체지향 프로그래밍을 개발자들에게 더 익숙하게 만들었습니다. 그러나 내부적으로 클래스는 여전히 프로토타입 상속을 기반으로 합니다.
...
[객체지향 프로그래밍에서 주요한 개념]
1. **속성 (Properties)**: 객체의 특징이나 성질을 나타내는 것입니다. 이는 객체가 가지는 데이터를 의미합니다.
2. **추상화 (Abstraction)**: 다양한 속성 중 프로그램에 필요한 속성만을 간추려내어 표현하는 과정입니다. 즉, 객체를 필요한 정보와 동작으로 단순화하는 것을 의미합니다.
3. **객체 (Object)**: 속성을 통해 여러 개의 값을 하나의 단위로 구성한 복합적인 자료구조입니다. 객체는 상태 데이터와 동작을 하나의 논리적 단위로 묶어서 표현합니다.
  - 상태 (State): 객체가 가지는 데이터를 나타냅니다. 객체의 속성은 상태를 나타내며, 이는 객체가 가지고 있는 데이터 값들입니다.
  - 동작 (Behavior): 객체의 상태 데이터를 조작하거나 다른 객체와의 상호작용을 통해 수행하는 동작을 나타냅니다. 객체의 동작은 메서드라는 함수 형태로 표현되며, 객체간의 상호작용을 통해 프로그램이 실행됩니다.
