...
블록 문은 제어문(Control flow statement; `if`, `for`, `while`), 함수 (Functions), 조건부 실행 (Conditional Execution), 지역 변수 (Local Variables)을 사용해야 할 상황일 때 함께 사용됩니다.
  - **제어문 (Control Statements)**:
    ...
    ```
  - **함수 (Functions)**: 함수 정의 시, 함수의 본문을 블록 문으로 감싸는 것이 일반적입니다. 이는 함수가 호출될 때 실행될 코드를 지정하는 데 사용됩니다.
    ```jsx
    function greet(name) {
      // 함수 본문
    ...
    ```
  - **조건부 실행 (Conditional Execution)**: 특정 조건에 따라 실행할 코드를 블록 문으로 그룹화하여 조건부로 실행할 수 있습니다.
    ```jsx
    if (condition) {
      // 조건이 참일 때 실행되는 코드
    } else {
    ...
    ```
  - **지역 변수 (Local Variables)**: 블록 문 내에서 선언된 변수는 해당 블록 문 내에서만 유효한 지역 변수로 취급됩니다. 이는 변수의 유효 범위를 제한하는 데 사용될 수 있습니다.
    ```jsx
...
- **선택적 표현식(Optional Expression)**
  - JavaScript에서 for 루프의 초기화식, 조건식, 증감식은 모두 선택적으로 사용할 수 있습니다. 특정한 상황에 따라 이러한 표현식을 생략할 수 있습니다.
    ...
    ```
    <details>
      <summary>무한 루프(infinite loop)</summary>
      무한 루프(infinite loop)는 반복문이 종료 조건을 충족하지 않고 무한히 반복되는 상황을 말합니다. 이는 프로그램이 원하는 대로 종료되지 않고 계속해서 실행되는 결과를 초래할 수 있습니다.

      무한 루프는 다양한 원인으로 발생할 수 있습니다. 주로 반복문의 종료 조건을 설정하지 않거나, 조건식이 항상 참(true)으로 평가되는 경우에 발생합니다. 예를 들어, 다음과 같은 코드는 무한 루프를 만들어낼 수 있습니다:
      ...
      ```
      위의 예시에서는 `while`, `for`, `do...while` 반복문의 조건식을 `true`로 설정하여 종료 조건을 충족하지 않도록 하였습니다. 이러한 코드는 반복문을 빠져나갈 조건이 없기 때문에 프로그램이 무한히 실행되는 무한 루프를 생성합니다. 이러한 상황에서는 프로그램이 응답하지 않고 멈추지 않으므로 주의해야 합니다.

      > 무한 루프를 피하기 위해서는 종료 조건이 없거나 항상 참으로 평가되는 조건식을 사용하지 않도록 주의해야 합니다.
    </details>
...
`continue` 문은 JavaScript에서 반복문 내에서 사용되며, 해당 반복문의 실행을 종료하지 않고 다음 반복으로 건너뛰는 역할을 합니다. 

`continue` 문은 사용된 반복문 유형에 따라 다음과 같이 동작합니다.
- `while` 또는 `do...while` 반복문에서는 `continue` 문이 실행되면 반복문의 조건 또는 반복문 블록의 시작점으로 이동합니다.
- `for` 반복문에서는 `continue` 문이 실행되면 업데이트 표현식으로 이동한 후, 반복문의 조건을 다시 평가하여 다음 반복을 진행합니다.
- `for...in`, `for...of`, 또는 `for await...of` 반복문에서는 `continue` 문이 실행되면 다음 반복으로 이동하여 반복문 블록 내의 남은 문장을 건너뜁니다.

또한, `continue` 문은 선택적인 레이블(label)과 함께 사용할 수 있습니다. 이는 가장 안쪽 반복문이 아닌 특정한 레이블이 지정된 반복문의 다음 반복으로 이동할 수 있도록 합니다. 이 경우 `continue` 문은 해당 레이블로 지정된 반복문 내에 중첩되어야 합니다.

중요한 점은, 레이블과 함께 있는지 여부에 상관없이 `continue` 문은 스크립트의 최상위 레벨, 모듈, 함수의 본문 또는 정적 초기화 블록에서 사용할 수 없습니다. continue 문은 반복문 내에서 사용되어야 합니다.
