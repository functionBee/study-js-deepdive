...
## 23-1. 소스코드의 타입
실행 컨텍스트는 ECMA Script(JavaScript)에서 코드 실행을 추적하고 관리하는 방법을 제공합니다.
...
각각의 소스 코드 타입은 실행 컨텍스트를 생성하는 과정과 내용에서 차이가 있습니다. 예를 들어, 전역 코드의 실행 컨텍스트는 스크립트 파일이 로드될 때 한 번만 생성되지만, 함수 코드의 실행 컨텍스트는 해당 함수가 호출될 때마다 생성됩니다. 또한, 스코프 체인은 각 실행 컨텍스트의 범위를 나타내며, 해당 소스 코드 타입에 따라 형성됩니다.

소스 코드의 타입에 따라 실행 컨텍스트를 구분하는 이유는 각 타입에 맞게 변수 및 함수의 스코프, 식별자 해결 방식 등을 관리하기 위해서입니다. 이를 통해 JavaScript 엔진은 코드 실행을 정확하게 추적하고, 올바른 변수와 함수를 참조할 수 있습니다.
...
1. 전역 변수 선언: `a`와 `b`는 전역 스코프에서 선언된 전역 변수입니다. 전역 실행 컨텍스트에서 변수 객체에 등록됩니다.
2. 함수 정의: `Sum` 함수는 전역 스코프에서 선언된 함수입니다. 함수 실행 컨텍스트가 생성되고, 변수 객체에 지역 변수 `a`와 `b`가 등록됩니다.
3. 함수 호출: `Sum(100)`을 호출하면 `Sum` 함수의 실행 컨텍스트가 생성되고, `x` 매개 변수에 값 100이 전달됩니다. 함수 내에서는 지역 변수 `a`와 `b`를 사용하여 계산을 수행하고 결과를 출력합니다.
4. 메서드 호출: `console.log(a + b)`는 전역 스코프에서 실행되는 코드입니다. 전역 변수 `a`와 `b`를 사용하여 계산을 수행하고 결과를 출력합니다.
...
- 실행 컨텍스트 스택(Execution Context Stack)은 실행 컨텍스트를 관리하는 자료구조로, 코드의 실행 순서와 관련된 정보를 저장하고 제어합니다.
